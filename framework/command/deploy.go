...
import (
	...
	"github.com/dll02/webgo/framework/provider/ssh"
...
var deployRollbackCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if len(args) != 2 {
			return errors.New("参数错误,请按照参数进行回滚 ./webgo deploy rollback [version] [frontend/backend/all]")
		}
...
func deployBuildBackend(c *cobra.Command, deployFolder string) error {
	...
	if err != nil {
		log.Fatalln("webgo go: 请在Path路径中先安装go")
	}
