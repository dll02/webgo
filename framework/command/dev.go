...
func (p *Proxy) newProxyReverseProxy(frontend, backend *url.URL) *httputil.ReverseProxy {
	...
	director := func(req *http.Request) {
		...
		fmt.Println(req.URL.Scheme + ":" + req.URL.Host)
	}
...
func (p *Proxy) rebuildBackend() error {
	// 重新编译webgo
	cmdBuild := exec.Command("./webgo", "build", "backend")
...
func (p *Proxy) restartBackend() error {
	...
	webgoAddress := fmt.Sprintf(":" + port)
	// 使用命令行启动后端进程
	cmd := exec.Command("./webgo", "app", "start", "--address="+webgoAddress)
