...
import (
	"github.com/dll02/webgo/framework/provider/log/formatter"
	"io"
	"strings"

	"github.com/dll02/webgo/framework"
	"github.com/dll02/webgo/framework/contract"
	"github.com/dll02/webgo/framework/provider/log/services"
)
...
// Register 注册一个服务实例
func (l *WebgoLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewWebgoConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewWebgoSingleLog
	case "rotate":
		return services.NewWebgoRotateLog
	case "console":
		return services.NewWebgoConsoleLog
	case "custom":
		return services.NewWebgoCustomLog
	default:
		return services.NewWebgoConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *WebgoLogServiceProvider) Boot(c framework.Container) error {
	return nil
}
