...
func NewWebgoTraceService(params ...interface{}) (interface{}, error) {
	...
	return &WebgoTraceService{idService: idService}, nil
}
...
func (t *WebgoTraceService) WithTrace(c context.Context, trace *contract.TraceContext) context.Context {
	if ginC, ok := c.(*gin.Context); ok {
		ginC.Set(string(TraceContextKey), trace)
		return ginC
	} else {
		newC := context.WithValue(c, TraceContextKey, trace)
		return newC
	}
}
...
func (t *WebgoTraceService) GetTrace(c context.Context) *contract.TraceContext {
	if ginC, ok := c.(*gin.Context); ok {
		if val, ok2 := ginC.Get(string(TraceContextKey)); ok2 {
			return val.(*contract.TraceContext)
		}
	}

	if tc, ok := c.Value(TraceContextKey).(*contract.TraceContext); ok {
		return tc
	}
	return nil
}
